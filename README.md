...
It sets CKAN up to be run directly on your local machine (not containerized) and thus it uses your local install of python and pip. As such, you'll need to configure your environment to adhere to the [basic system requirements](#requirements) yourself. Although the solr, redis, and postgres containers will all be started automatically, CKAN itself will not; see [Running](#running) for more.
